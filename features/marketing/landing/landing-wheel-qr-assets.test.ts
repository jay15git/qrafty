...
import { renderLandingWheelCardSvg } from "@/features/marketing/landing/landing-wheel-qr-render";
...
describe("landing wheel qr assets", () => {
  it("matches baked svg assets", () => {
    ...
    for (const preset of LANDING_WHEEL_CARD_PRESETS) {
      ...
      expect(fs.existsSync(filePath), `missing wheel asset: ${preset.id}.svg`).toBe(true);
