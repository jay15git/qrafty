...
function createBrandIconSvgMarkup(brandIcon: BrandIconEntry, color: string) {
...
export function createBrandIconDataUrl(brandIcon: BrandIconEntry, color: string) {
  return svgMarkupToDataUrl(createBrandIconSvgMarkup(brandIcon, color));
}

function createBrandIconGradientSvgMarkup(brandIcon: BrandIconEntry, gradient: QraftyGradient) {
  ...
  return injectSvgDefinitions(baseMarkup, gradientMarkup).replaceAll("currentColor", fillValue);
}
