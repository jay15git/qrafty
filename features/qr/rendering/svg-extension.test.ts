  ...
  const element: StubElement = {
    ...
    querySelectorAll(selector) {
      const matches = (node: StubElement) => {
        ...
        if (["feComposite", "feFlood", "feGaussianBlur", "feOffset"].includes(selector)) {
...
function getGradientRelativeCoordinates(gradient: StubElement, fillRect: StubElement) {
...
describe("qr rendering helpers", () => {
  ...
  it("applies palette module colors as qr dot layers", () => {
    ...
    expect([
      ...new Set(
        colorLayers.map(
          (layer) => layer.getAttribute("fill") ?? layer.children[0]?.getAttribute("fill"),
        ),
      ),
  ...
  it("adds a background image layer to the svg extension output", () => {
    ...
    expect(backgroundImage?.getAttribute("href")).toBe("blob:https://qrafty.local/background.png");
  ...
  it("clips background images with the configured qr background radius", () => {
    ...
    expect(backgroundImage?.getAttribute("clip-path")).toBe("url('#clip-path-background-image')");
  ...
  it("adds a solid vector background shape fitted to the svg viewport", () => {
    ...
    expect(backgroundShape?.getAttribute("transform")).toBe("translate(0 33) scale(1)");
  ...
  it("expands vector background shape bounds with padding and stroke", () => {
    ...
    expect(qrContent?.getAttribute("transform")).toBe("translate(24 24) scale(0.85)");
    expect(qrContent?.children).toContain(qrPath);
    expect(backgroundShape?.getAttribute("transform")).toBe("translate(0 0) scale(1)");
  ...
  it("expands the default qr background surface with padding and stroke", () => {
    ...
    expect(svg.getAttribute("height")).toBe("320");
    expect(svg.getAttribute("viewBox")).toBe("0 0 320 320");
    expect(qrContent?.getAttribute("transform")).toBe("translate(20 20) scale(0.875)");
    expect(qrContent?.children).toContain(qrPath);
    ...
    expect(svg.children[1]?.getAttribute("data-qr-layer")).toBe("background-surface-stroke");
  ...
  it("normalizes all corner-frame linear gradients to the same relative direction", () => {
    ...
    const topLeftCoordinates = getGradientRelativeCoordinates(topLeft.gradient, topLeft.fillRect);

    expect(getGradientRelativeCoordinates(topRight.gradient, topRight.fillRect)).toEqual(
      topLeftCoordinates,
    );
    expect(getGradientRelativeCoordinates(bottomLeft.gradient, bottomLeft.fillRect)).toEqual(
      topLeftCoordinates,
    );
  });
