...
export function getCellSpaceBackgroundMetrics(
  ...
  return {
    metrics: getBackgroundRenderMetrics(numCells, numCells, cellShapeOptions, {
...
export function normalizeBackgroundShapeOptions(
  ...
  const numField = (
    ...
    return normalize(
      (options?.[key] as number | undefined) ?? fallbackOverride ?? fallback,
      fallback,
    );
  };
...
function getBackgroundRenderMetrics(
  ...
  const quietZoneFraction = Math.min(0.49, Math.max(0, layout?.quietZoneFraction ?? 0));
  ...
  const contentTarget = Math.max(MIN_QR_CONTENT_TARGET, frameSize - paddingPx * 2);
...
export function getBackgroundShapeTransform(
  ...
  const scale = Math.min(region.width / shape.viewBox.width, region.height / shape.viewBox.height);
