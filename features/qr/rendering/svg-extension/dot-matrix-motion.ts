...
import { getSvgShapeBounds } from "./svg-shape-bounds";
import {
...
function materializeDataModulePaths(
  ...
  if (
    unifiedImage &&
    isSvgElementLike(unifiedImage) &&
    unifiedImage.tagName.toLowerCase() === "image"
  ) {
  ...
  for (const layer of pathLayers) {
    ...
    for (const segmentShape of layer.shapes) {
      ...
      path.setAttribute("fill", resolveMotionModuleFill(segmentShape, layer.fill, state));
...
function collectCanvasDotModuleShapes(svg: SVGElement): SVGElement[] {
  const fromMaterialized = [
    ...
    ...svg.querySelectorAll('[data-qr-layer="dot-palette"] [data-qr-layer="dot-palette-fill"] > *'),
