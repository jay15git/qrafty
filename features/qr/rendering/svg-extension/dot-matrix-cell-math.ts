...
export function getDotMatrixRegionCoordinate(value: number, matrixSize: number) {
  return Math.min(matrixSize - 1, Math.max(0, Math.floor(clampDotMatrixUnit(value) * matrixSize)));
}
...
export function indexToCoord(
  index: number,
  matrixSize = DEFAULT_DOT_MATRIX_TILE_SIZE,
): DotMatrixCell {
  return {
...
export function getDotMatrixPatternIndexes(
