...
import { StylePreview, type StylePreviewKind } from "@/features/qr/components/StylePreview";
...
function normalizeBakedStylePreviewSvg(markup: string, previewKind: StylePreviewKind) {
  let svg = markup;
  ...
  svg = svg.replace(/<svg\b([^>]*)>/, (_match, attrs: string) => {
    const cleaned = attrs.replace(/\s*height="[^"]*"/g, "").replace(/\s*width="[^"]*"/g, "");
...
export function renderBakedStylePreviewSvg(previewKind: StylePreviewKind, value: string) {
