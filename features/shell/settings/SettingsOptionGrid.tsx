...
import {
  settingsOptionGridClass,
  type SettingsOptionGridColumns,
  type SettingsOptionGridSpacing,
...
function settingsOptionRowClass(className?: string) {
  return cn(
...
const SETTINGS_OPTION_SELECTION_APPEAR: Transition = {
  opacity: { duration: 0.38, ease: [0.4, 0, 0.2, 1] },
  filter: { duration: 0.42, ease: [0.4, 0, 0.2, 1] },
};

const SETTINGS_OPTION_SELECTION_EXIT: Transition = {
  opacity: { duration: 0.24, ease: [0.4, 0, 1, 1] },
  filter: { duration: 0.24, ease: [0.4, 0, 1, 1] },
};

const SETTINGS_FROZEN_MOTION_TRANSITION: Transition = { duration: 0 };

type SettingsOptionSelectionRect = {
  height: number;
  left: number;
  top: number;
  width: number;
...
type SettingsOptionSelection = {
  key: string;
  rect: SettingsOptionSelectionRect;
};

function measureSettingsOptionSelection(
  container: HTMLElement,
): SettingsOptionSelectionRect | null {
...
function resolveSettingsOptionSelectionKey(
...
export function SettingsAnimatedOptionGrid({
  className,
  columns,
  children,
  layout = "grid",
  ...
  className?: string;
  columns?: SettingsOptionGridColumns;
  children: ReactNode;
  layout?: "grid" | "row";
  selectedKey?: string | number | boolean | null;
} & ComponentProps<"div">) {
  const containerRef = useRef<HTMLDivElement>(null);
  const [selection, setSelection] = useState<SettingsOptionSelection | null>(null);
  ...
  const measureSelected = useCallback(() => {
    ...
    const rect = measureSettingsOptionSelection(container);
    const key = resolveSettingsOptionSelectionKey(selectedKey);
  ...
  return (
    <div
      ...
      className={cn(
        "relative",
        layout === "row"
          ? settingsOptionRowClass(className)
          : settingsOptionGridClass(columns ?? 3, className),
      )}
      {...props}
    >
            ...
            exit={
              skipAppear
                ...
                : {
                    opacity: 0,
                    filter: "blur(8px)",
                    transition: SETTINGS_OPTION_SELECTION_EXIT,
                  }
            }
            transition={
              skipAppear ? SETTINGS_FROZEN_MOTION_TRANSITION : SETTINGS_OPTION_SELECTION_APPEAR
            }
          />
        ) : null}
      </AnimatePresence>
...
export type SettingsOptionGridRowKind =
  "square" | "labeled" | "h-12" | "h-10" | "h-9" | "h-8" | "content";

export type SettingsOptionGridVariant = "preset" | "compact" | "content";

const GRID_ROW_HEIGHT_HORIZONTAL: Record<SettingsOptionGridRowKind, string> = {
  square: "h-[5.75rem]",
  labeled: "h-[5.75rem]",
  "h-12": "h-14",
  "h-10": "h-12",
...
const GRID_ROW_HEIGHT_VERTICAL: Record<SettingsOptionGridRowKind, string> = {
  "h-12": "h-[10.5rem]",
  "h-10": "h-[7.125rem]",
  "h-9": "h-[6.75rem]",
  "h-8": "h-[6.375rem]",
...
function defaultRowKind(
  variant: SettingsOptionGridVariant,
  columns: SettingsOptionGridColumns,
  orientation: "vertical" | "horizontal",
): SettingsOptionGridRowKind {
...
function optionGridScrollHeightClass({
  ...
  columns?: SettingsOptionGridColumns;
  orientation?: "vertical" | "horizontal";
  rowKind?: SettingsOptionGridRowKind;
  variant: SettingsOptionGridVariant;
}): string {
...
export function SettingsScrollArea({ children }: { children: ReactNode }) {
  return (
    <ScrollArea
      chevron
      cueSize="comfortable"
      data-slot="settings-scroll-area"
      persistKey="settings-body"
      scrollFade
...
export function SettingsOptionGridScrollArea({
  ariaLabel,
  children,
  className,
  columns = 3,
