...
} from "@/features/shell/inspector/SettingsFillOptionGrid";
import {
...
/** Inspector accordion row: each palette swatch opens its own picker. */
function PatternRowSwatch({
  ...
  return (
    ...
    >
      <button aria-label={`Edit color ${index + 1}`} className={PATTERN_ROW_SWATCH} type="button">
...
function QrColorFillModeContent({
  ...
  if (modeTab === "Image" && moduleImage) {
    return <QrColorFillImageSection moduleImage={moduleImage} persistKey={persistKey} />;
  }
...
export function QrColorFillControls({
  ...
  const [modeTab, setModeTab] = useState<QrColorFillModeTab>(() =>
    moduleCapable ? moduleFillModeToTab(moduleFillMode ?? "solid", value) : fillValueToTab(value),
  );
