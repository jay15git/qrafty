...
function useSettingsSlice<T extends object>(defaults: T) {
  ...
  const onChange = (patch: Partial<T>) => setValue((current) => ({ ...current, ...patch }));
...
function useSettingsSlices() {
  ...
  const [accessibilitySettings, onAccessibilityPatch] = useSettingsSlice(
    DEFAULT_DESKTOP_ACCESSIBILITY_SETTINGS,
  );
  ...
  const [layersSettings, setLayersSettings] = useState<LayersSettings>(DEFAULT_LAYERS_SETTINGS);
...
export function useToolbarSettingsModel({
  ...
  const overrideFields = <
    Entries extends readonly (readonly [string, keyof ToolbarController, unknown])[],
  >(
    entries: Entries,
  ) =>
    Object.fromEntries(
      ...
      [K in Entries[number][0] & keyof SettingsModel]: SettingsModel[K];
    };
