...
export function createQrControls(controls: QrControls) {
  ...
  const syncLogo = (nextState: QraftyState) => {
    ...
    controls.setSelectedLogoColor(nextState.logo.presetColor ?? DEFAULT_BRAND_ICON_COLOR);
    controls.setSelectedLogoColorMode(nextState.logoGradient.enabled ? "gradient" : "solid");
    controls.setSelectedLogoGradient(structuredClone(nextState.logoGradient));
    controls.setSelectedLogoSize(Math.round(nextState.imageOptions.imageSize * 100));
    controls.setSelectedLogoMargin(nextState.imageOptions.margin);
    controls.setSelectedHideBackgroundDots(nextState.imageOptions.hideBackgroundDots);
  ...
  const syncDots = (nextState: QraftyState) => {
    ...
    controls.setSelectedDotsGradient(structuredClone(nextState.dataModulesGradient));
    controls.setSelectedDotMatrixAnimation({ ...nextState.dotMatrixAnimation });
  };

  const syncCorners = (nextState: QraftyState) => {
    controls.setSelectedQrFinderPatternOuterStyle(nextState.finderPatternOuterSettings.type);
    controls.setSelectedCornerSquareColorMode(
      nextState.finderPatternOuterGradient.enabled ? "gradient" : "solid",
    );
    controls.setSelectedCornerSquareColor(nextState.finderPatternOuterSettings.color);
    controls.setSelectedCornerSquareGradient(structuredClone(nextState.finderPatternOuterGradient));
    controls.setSelectedQrFinderPatternInnerStyle(nextState.finderPatternInnerSettings.type);
    ...
    controls.setSelectedCornerDotGradient(structuredClone(nextState.finderPatternInnerGradient));
  };
  ...
  const syncBackground = (nextState: QraftyState) => {
    ...
    controls.setSelectedBackgroundTransparent(nextState.backgroundOptions.transparent);
    controls.setSelectedBackgroundGradient(structuredClone(nextState.backgroundGradient));
  ...
  const applyQrState = (nextState: QraftyState) => {
    ...
    controls.setSelectedQrRadius(clampQrBackgroundRound(nextState.backgroundOptions.round));
    controls.setSelectedRasterExportQualityPercent(nextState.rasterExportQualityPercent);
    ...
    controls.setSelectedQrErrorCorrectionLevel(nextState.qrOptions.errorCorrectionLevel);
    controls.setSelectedBoostLevel(nextState.qrOptions.boostLevel);
    controls.setSelectedQrMode(nextState.qrOptions.mode);
    controls.setSelectedValueSegmentsText(formatValueSegmentsText(nextState.valueSegments));
