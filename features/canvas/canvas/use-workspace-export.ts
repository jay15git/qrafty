    ...
    try {
      ...
      const onProgress = (progress: WorkspaceExportProgress) => {
        if (progress.kind === "video") {
          setProgressLabel(`Encoding video ${progress.frameIndex}/${progress.frameCount}...`);
          setProgressRatio(
      ...
      const runExport = (
        overrides: Pick<Parameters<typeof runWorkspaceExport>[0], "layers" | "name" | "state">,
      ) =>
        runWorkspaceExport({
      ...
      } else if (downloadTarget === "current" || downloadTarget.startsWith("qr:")) {
        const layerId =
          downloadTarget === "current" ? activeQrLayerId : downloadTarget.slice("qr:".length);
        const exportState = layerId === activeQrLayerId ? state : qrStateByLayerId[layerId];
