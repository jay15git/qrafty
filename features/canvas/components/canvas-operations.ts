...
export function getExportTarget(target: CanvasDownloadTarget): ExportTarget {
...
export function toLayerRow(layer: CanvasLayer): LayerRow {
  return {
    blur: layer.blur,
...
export function patchDraftingLayerById(
  ...
  if (layer.id === layerId) {
    return patchCanvasLayer(layer, patch);
  }
...
export function parseDraftingLayerClipboardPayload(value: string) {
  try {
    ...
    return {
      bounds,
      layers: payload.layers as CanvasLayer[],
