...
describe("DraftingCanvas", () => {
  it("deletes removable selected layers from the floating canvas toolbar", async () => {
    buildDraftingQraftyMarkupSpy.mockReturnValue(QR_PAYLOAD.markup);
    const canvas = renderCanvas({ paneToolbarVariant: "zoom" });

    await waitForDraftingCanvas();
    ...
    expect(canvas.container.querySelector('[data-slot="drafting-text-layer"]')).not.toBeNull();
    expect(
      canvas.container
        .querySelector('[data-slot="drafting-text-layer"]')
        ?.getAttribute("data-selected"),
    ).toBe("true");

    act(() => {
      activateElement(
        getRequiredElement(canvas.container, 'button[aria-label="Delete selection"]'),
      );
    });
    ...
    expect(canvas.container.querySelector('[data-slot="drafting-text-layer"]')).toBeNull();
    expect(canvas.container.querySelector('[data-slot="desktop-compose-node"]')).not.toBeNull();
    expect(
      canvas.container.querySelector('[data-slot="drafting-layer-floating-toolbar"]'),
    ).toBeNull();
  });
  ...
  it("renders the desktop chrome across the dynamic island, utility toolbar, and settings panel", async () => {
    ...
    expect(getRequiredElement(dynamicIsland, '[data-slot="canvas-size-trigger"]')).not.toBeNull();
    expect(getRequiredElement(dynamicIsland, '[data-slot="insert-trigger"]')).not.toBeNull();
    expect(dynamicIsland.querySelector('[data-slot="undo-trigger"]')).toBeNull();
    expect(dynamicIsland.querySelector('[data-slot="redo-trigger"]')).toBeNull();
    ...
    const utilityToolbar = getRequiredElement(canvas.container, '[data-slot="utility-toolbar"]');
    expect(canvas.container.querySelector('[data-slot="document-toolbar"]')).toBeNull();
    expect(
    ...
    expect(
      Array.from(settingsHeader.querySelectorAll("button")).map((button) =>
        button.getAttribute("aria-label"),
      ),
    ).toEqual(["Undo", "Redo"]);
    ...
    expect(
      getRequiredElement(settingsFooter, '[data-slot="keyboard-shortcuts-trigger"]'),
    ).not.toBeNull();
    expect(
      getRequiredElement(settingsFooter, '[data-slot="theme-toggle"]').getAttribute("aria-label"),
    ).toBe("Switch to light mode");

    expect(canvas.container.querySelector('[data-slot="action-toolbar"]')).toBeNull();
    expect(canvas.container.querySelector('[data-slot="desktop-history-actions"]')).toBeNull();
    expect(canvas.container.querySelector('[data-slot="desktop-compose-toolbar"]')).toBeNull();
    expect(
      canvas.container.querySelector('[data-slot="desktop-compose-toolbar-anchor"]'),
    ).toBeNull();
    expect(canvas.container.querySelector('[data-slot="resize-toolbar"]')).toBeNull();
    expect(canvas.container.querySelector('[data-slot="zoom-popover"]')).toBeNull();
  });
  ...
  it("opens keyboard shortcuts from the settings panel footer", () => {
    ...
    expect(document.body.querySelector('[data-slot="keyboard-shortcuts-popover"]')).toBeNull();
  ...
  it("does not expose the dashboard edit mode toggle or edit rail on drafting", async () => {
    ...
    expect(canvas.container.querySelector('button[aria-label="Toggle edit mode"]')).toBeNull();
    expect(canvas.container.querySelector('[data-slot="desktop-compose-edit-mode"]')).toBeNull();
    expect(canvas.container.querySelector('[data-slot="dashboard-edit-rail"]')).toBeNull();
    expect(
      canvas.container.querySelector('[data-slot="drafting-edit-nav-scroll-area"]'),
    ).toBeNull();
    expect(canvas.container.querySelector('[data-slot="drafting-edit-panel-scroll"]')).toBeNull();
    expect(canvas.container.querySelector('[data-slot="dashboard-edit-page"]')).toBeNull();
    expect(canvas.container.querySelector('[data-slot="drafting-header"]')).toBeNull();
    expect(canvas.container.querySelector('[data-slot="drafting-nav"]')).toBeNull();
    expect(canvas.container.querySelector('[data-drafting-tool-button="true"]')).toBeNull();
  });

  it("does not render a document guide overlay in the default compose canvas", () => {
    const canvas = renderCanvas();

    expect(
      canvas.container.querySelector('[data-slot="desktop-compose-document-guides"]'),
    ).toBeNull();
  });
  ...
  it("adds a fresh qr layer from the bottom toolbar and selects it", async () => {
    ...
    expect(canvas.container.querySelectorAll('[data-slot="desktop-compose-surface"]')).toHaveLength(
      1,
    );
    expect(canvas.container.querySelectorAll('[data-slot="desktop-compose-node"]')).toHaveLength(1);
    ...
    expect(canvas.container.querySelectorAll('[data-slot="desktop-compose-node"]')).toHaveLength(2);
    expect(
      getRequiredElement(canvas.container, '[data-slot="drafting-surface"]').getAttribute(
  ...
  it("keeps independent qr layer content on one canvas", async () => {
    ...
    const qrNodes = Array.from(
      canvas.container.querySelectorAll('[data-slot="desktop-compose-node"]'),
    ) as HTMLElement[];
    ...
    expect(qrNodes.map((node) => node.getAttribute("data-node-id"))).toEqual(
      expect.arrayContaining(["https://example.com/first", "https://example.com/second"]),
    );
  });
  ...
  it("renders a faint dotted texture behind the neutral pane workspace", async () => {
    ...
    await waitForDraftingCanvas();
    const composeCanvas = getRequiredElement(
      canvas.container,
      '[data-slot="desktop-compose-surface"]',
    );

    expect(composeCanvas.getAttribute("data-canvas-appearance")).toBe("workspace");
    expect(canvas.container.querySelector('[data-slot="drafting-surface"]')).not.toBeNull();
  });
  ...
  it("uses keyboard shortcuts for undo and redo without intercepting text input undo", async () => {
    ...
    act(() => {
      getRequiredElement(canvas.container, '[data-slot="drafting-surface"]').dispatchEvent(
  ...
  it("uses keyboard shortcuts to select all, clear selection, order layers, delete cards, and keep the canonical QR", async () => {
    ...
    const pastedQrLayer = Array.from(
      canvas.container.querySelectorAll<HTMLElement>('[data-slot="desktop-compose-node"]'),
    ...
    expect(
      Array.from(canvas.container.querySelectorAll<HTMLElement>("[data-layer-id]"))
        .filter((layer) => layer.dataset.selected === "true")
        .map((layer) => layer.dataset.layerId)
        .filter(Boolean),
    ).toEqual(expect.arrayContaining(["dashboard-qr-node:qr", pastedQrLayer?.dataset.layerId]));
    ...
    expect(
      Array.from(canvas.container.querySelectorAll<HTMLElement>("[data-layer-id]")).some(
  ...
  it("uses keyboard shortcuts to group and ungroup selected layers", async () => {
    ...
    let groupLayer = getRequiredElement(canvas.container, '[data-slot="drafting-layer-group"]');
    ...
    expect(canvas.container.querySelector('[data-slot="drafting-layer-group"]')).toBeNull();
    ...
    groupLayer = getRequiredElement(canvas.container, '[data-slot="drafting-layer-group"]');
  ...
  it("focuses the drafting canvas after selecting a canvas layer", async () => {
    ...
    const draftingCanvas = getRequiredElement(canvas.container, '[data-slot="drafting-surface"]');
    const qrLayer = getRequiredElement(canvas.container, '[data-slot="desktop-compose-node"]');
  ...
  it("keeps add QR and reset changes undoable", async () => {
    ...
    act(() => {
      activateElement(getRequiredElement(canvas.container, '[data-slot="undo-trigger"]'));
    });
    ...
    act(() => {
      activateElement(getRequiredElement(canvas.container, '[data-slot="redo-trigger"]'));
    });
  ...
  it("restores the autosaved drafting workspace after remount", async () => {
    const firstCanvas = renderCanvas({ paneToolbarVariant: "zoom" });
    ...
    firstCanvas.unmount();
    const secondCanvas = renderCanvas({ paneToolbarVariant: "zoom" });
    await waitForDraftingCanvas();
    await advanceDraftingTimers();

    expect(
      getRequiredElement(secondCanvas.container, '[data-slot="drafting-surface"]').getAttribute(
        "data-qr-content-value",
      ),
...
function OverlayTestHarness(props: ComponentProps<typeof DraftingCanvas>) {
    ...
    <CuelumeProvider>
      <DraftingCanvas
        ...
        renderOverlay={(controller) => (
          <FloatingToolbar controller={controller} onThemeChange={setTheme} theme={theme} />
        )}
      />
    </CuelumeProvider>
  );
...
function renderCanvas(props: ComponentProps<typeof DraftingCanvas> = {}) {
  const container = document.createElement("div");
  const root = createRoot(container);

  act(() => {
    root.render(<OverlayTestHarness {...props} />);
  });
...
function getAccordionTriggerByText(parent: ParentNode, text: string) {
  const trigger = Array.from(parent.querySelectorAll('[data-slot="drafting-color-trigger"]')).find(
...
async function waitForDraftingCanvas() {
  await act(async () => {
...
async function addQrCode(parent: ParentNode) {
  await act(async () => {
    activateElement(getRequiredElement(parent, '[data-slot="insert-trigger"]'));
    await flushPromises();
  });
  await act(async () => {
