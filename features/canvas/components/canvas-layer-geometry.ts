...
type LayerBounds = Pick<CanvasLayer, "height" | "id" | "width" | "x" | "y">;
...
function anchorSquareLayerResize(
  layer: CanvasLayer,
  direction: ResizeDirection,
  geometry: Pick<CanvasLayer, "height" | "width" | "x" | "y">,
): Pick<CanvasLayer, "height" | "width" | "x" | "y"> {
  const affectsWest = direction.includes("w");
  const affectsEast = direction.includes("e");
  const affectsNorth = direction.includes("n");
  ...
  return {
    height: size,
    width: size,
    x: affectsWest ? right - size : affectsEast ? geometry.x : layer.x + (layer.width - size) / 2,
    y: affectsNorth
      ? bottom - size
      : affectsSouth
        ? geometry.y
...
export function getCombinedLayerBounds(layers: CanvasLayer[]) {
  if (layers.length === 0) {
    return null;
  }
...
function getLayerRotatedCorners(layer: CanvasLayer) {
  const center = {
...
function getCommonLayerRotation(layers: CanvasLayer[]) {
  const [firstLayer] = layers;
  const firstRotation = normalizeLayerRotation(firstLayer?.rotation ?? 0);

  return layers.every(
    (layer) => Math.abs(normalizeLayerRotation(layer.rotation) - firstRotation) < 0.001,
  )
    ? firstRotation
    : null;
}
...
export function getMarqueeBounds(start: { x: number; y: number }, end: { x: number; y: number }) {
...
function snapResizeAxis({
  ...
  let best: { distance: number; edge: "start" | "end"; guide: number; offset: number } | null =
    null;
...
function getSnapTargets(layers: CanvasLayer[], activeLayerId: string, axis: SnapAxis) {
  const targets = [0];
