...
import { getChromeFrameRect } from "@/features/canvas/components/pane-layer-chrome-overlay";
import {
...
describe("Pane", () => {
  ...
  it("renders the editable card layer behind the qr artwork", async () => {
    ...
    const card = container.querySelector('[data-slot="desktop-compose-card"]') as HTMLElement;
    const cardFill = container.querySelector(
      '[data-slot="desktop-compose-card-fill"]',
    ) as HTMLElement;
    const cardBorder = container.querySelector(
  ...
  it("renders selected qr backing shapes as a qr-layer background without changing the card", async () => {
    ...
    const qrComponent = container.querySelector(
  ...
  it("snaps moving layers to nearby layer center guides", async () => {
    ...
    expect(
      container.querySelector('[data-slot="drafting-layer-snap-guide"][data-axis="vertical"]'),
    ).not.toBeNull();
  });
  ...
  it("keeps the text editor active after double click without a floating format toolbar", async () => {
    ...
    const { container, reactRoot } = renderPane(
      createDefaultQraftyState(),
      true,
      createDefaultDraftingCardState(),
      {
  ...
  it("renders legacy text runs for display", async () => {
    ...
    const runs = Array.from(
      text.querySelectorAll('[data-slot="drafting-text-run"]'),
    ) as HTMLElement[];
  ...
  it("shows floating layer actions for a selected text layer", async () => {
    ...
    expect(document.querySelector('[data-slot="drafting-layer-text-align-settings"]')).toBeTruthy();
  ...
  it("keeps the rotation value visible for two seconds after rotation ends", async () => {
    ...
    expect(
      container.querySelector('[data-slot="drafting-layer-rotation-value"]')?.textContent,
    ).toBe("90°");
  ...
  it("moves every selected layer without collapsing the combined selector", async () => {
    ...
    expect(
      container.querySelector('[data-slot="drafting-layer-multi-select-frame"]'),
    ).not.toBeNull();
  });
