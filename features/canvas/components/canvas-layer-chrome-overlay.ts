...
export function getChromeFrameRect(bounds: ChromeBounds, paddingPx: number, space: ChromeSpace) {
...
export function getFloatingLayerToolbarPosition({
  bounds,
  canvasHeight,
  canvasWidth,
  gapPx,
  ...
  const bottomBoundary = canvasHeight > 0 ? canvasHeight / 2 - toolbarHeightPx - gutterPx : rawY;
