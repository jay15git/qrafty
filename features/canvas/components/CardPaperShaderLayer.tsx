...
function CanvasCardPaperShaderRenderer({
  ...
  }, [captureFrames, mountGeneration, onFrame, paperShader.shaderId, playbackSpeed, shouldAnimate]);

  return (
    <PaperShaderErrorBoundary key={`${paperShader.shaderId}:${mountGeneration}`} onError={onError}>
...
export const CanvasCardPaperShaderLayer = memo(
  function CanvasCardPaperShaderLayer({
    ...
    const isPaused = hasPlayback && (paperShader.paused || paperShader.speed === 0);
