...
export function getDraftingQrBackgroundSvgMarkup(layer: DraftingCanvasLayer, state: QraftyState) {
...
function buildDraftingQrBackgroundPreviewSvgMarkup(layer: DraftingCanvasLayer, state: QraftyState) {
  if (!shouldRenderDraftingQrBackground(state)) {
    return null;
  }
...
function getDraftingQrBackgroundDefsMarkup(ids: DraftingQrBackgroundIds, state: QraftyState) {
  const parts: string[] = [];
