...
vi.mock("@/features/qr/components/DotMatrixAnimatedQr", () => ({
  DotMatrixAnimatedQr: ({ canvasSvgMarkup }: { canvasSvgMarkup?: string | null }) => (
    <div data-canvas-markup={canvasSvgMarkup ?? ""} data-testid="dot-matrix-animated-qr" />
  ),
}));
...
describe("Artboard", () => {
  it("reuses cached markup for an equal state and rebuilds when state changes", async () => {
    ...
    const thirdState = {
      ...firstState,
      width: clampQrSize(firstState.width + 40),
      height: clampQrSize(firstState.width + 40),
    };

    const { container, reactRoot } = renderArtboard(firstState);
  ...
  it("lets the qr canvas fill the preview board", async () => {
    ...
    const { container } = renderArtboard(state);
  ...
  it("keeps the qr layer box at the state size when the background shape adds padding and stroke", async () => {
    ...
    const card = container.querySelector('[data-slot="canvas-card"]') as HTMLElement;
    const node = container.querySelector('[data-slot="canvas-node"]') as HTMLElement;
  ...
  it("renders the editable card layer behind the qr artwork", async () => {
    ...
    const card = container.querySelector('[data-slot="canvas-card"]') as HTMLElement;
    const cardFill = container.querySelector('[data-slot="canvas-card-fill"]') as HTMLElement;
    const cardBorder = container.querySelector('[data-slot="canvas-card-border"]') as HTMLElement;
    const node = container.querySelector('[data-slot="canvas-node"]') as HTMLElement;
  ...
  it("renders selected qr backing shapes as a qr-layer background without changing the card", async () => {
    ...
    const card = container.querySelector('[data-slot="canvas-card"]') as HTMLElement;
    const cardShape = card.querySelector('[data-slot="canvas-card-shape"]');
    const qrBackground = container.querySelector('[data-slot="canvas-qr-background"]');
  ...
  it("shows corner resize handles and grabbable edge zones for the selected layer", async () => {
    ...
    const cornerDirections = Array.from(
      container.querySelectorAll('[data-slot="canvas-layer-resize-handle"]'),
    ).map((handle) => handle.getAttribute("data-resize-direction"));
    const edgeDirections = Array.from(
      container.querySelectorAll('[data-slot="canvas-layer-resize-edge"]'),
    ).map((handle) => handle.getAttribute("data-resize-direction"));
    const resizeHandle = container.querySelector('[data-slot="canvas-layer-resize-handle"]');
    const resizeKnob = container.querySelector('[data-slot="canvas-layer-resize-handle-knob"]');
  ...
  it("snaps moving layers to nearby layer center guides", async () => {
    ...
    expect(
      container.querySelector('[data-slot="canvas-layer-snap-guide"][data-axis="vertical"]'),
    ).not.toBeNull();
  });
  ...
  it("keeps moving layers freeform outside the snap threshold", async () => {
    ...
    expect(container.querySelector('[data-slot="canvas-layer-snap-guide"]')).toBeNull();
  });
  ...
  it("snaps resize handles to nearby layer edges", async () => {
    ...
    const handle = container.querySelector(
      '[data-slot="canvas-layer-resize-edge"][data-resize-direction="e"]',
  ...
  it("renders selected layer controls above higher z-index layer content", async () => {
    ...
    expect(frame.style.zIndex).toBe("10000");
    expect(
      frame.compareDocumentPosition(upperLayer) & Node.DOCUMENT_POSITION_PRECEDING,
    ).toBeTruthy();
  });
  ...
  it("shows a rotation handle above the selected layer controls", async () => {
    ...
    const rotateHandle = container.querySelector('[data-slot="canvas-layer-rotate-handle"]');
  ...
  it("renders and edits Avnac-style text layers inline", async () => {
    ...
    const text = getRequiredElement(container, '[data-slot="canvas-text-layer"]') as HTMLElement;
    const content = getRequiredElement(text, '[data-slot="canvas-text-content"]') as HTMLElement;
    ...
    const editor = getRequiredElement(
      text,
  ...
  it("keeps the text editor active after double click without a floating format toolbar", async () => {
    ...
    const onLayerChange = vi.fn((layerId: string, patch: Partial<CanvasLayer>) => {
    ...
    const activeEditor = getRequiredElement(
      container,
      '[data-slot="canvas-text-editor"]',
    ) as HTMLTextAreaElement;
  ...
  it("renders legacy text runs for display", async () => {
    ...
    const runs = Array.from(
      text.querySelectorAll('[data-slot="canvas-text-run"]'),
    ) as HTMLElement[];
  ...
  it("shows floating layer actions for a selected text layer", async () => {
    ...
    const toolbar = getRequiredElement(container, '[data-slot="canvas-layer-floating-toolbar"]');
    ...
    expect(document.querySelector('[data-slot="canvas-layer-text-align-settings"]')).toBeTruthy();

    act(() => {
      clickElement(getRequiredElement(toolbar, 'button[aria-label="Text size"]'));
    });

    expect(document.querySelector('[data-slot="canvas-layer-text-size-settings"]')).toBeTruthy();
    const textSizeSlider = document.querySelector(
      '[data-slot="canvas-layer-text-size-settings"] [role="slider"][aria-label="Size"]',
    );
  ...
  it("opens the existing context menu from the floating more button", async () => {
    ...
    const moreButton = getRequiredElement(
      container,
      '[data-slot="canvas-layer-floating-toolbar"] button[aria-label="More layer actions"]',
    );
  ...
  it("closes the layer context menu after clicking outside it", async () => {
    ...
    expect(document.body.querySelector('[data-slot="canvas-layer-context-menu"]')).not.toBeNull();

    act(() => {
      document.body.dispatchEvent(new MouseEvent("pointerdown", { bubbles: true, button: 0 }));
    });

    expect(document.body.querySelector('[data-slot="canvas-layer-context-menu"]')).toBeNull();
  });
  ...
  it("opens an empty canvas context menu without paste actions", async () => {
    ...
    const canvas = getRequiredElement(container, '[data-slot="canvas"]');
  ...
  it("keeps the rotation value visible for two seconds after rotation ends", async () => {
    ...
    expect(container.querySelector('[data-slot="canvas-layer-rotation-value"]')?.textContent).toBe(
      "90°",
    );
  ...
  it("renders padded resize and rotate controls around multiple selected layers", async () => {
    ...
    expect(container.querySelector('[data-slot="canvas-layer-resize-frame"]')).not.toBeNull();
    expect(container.querySelector('[data-slot="canvas-layer-multi-select-frame"]')).toBeNull();
  });
  ...
  it("resizes every selected layer from the combined resize handles", async () => {
    ...
    const { container } = renderArtboard(createDefaultQraftyState(), true, TEST_CANVAS_CARD_STATE, {
      layers,
      onLayerChange,
  ...
  it("moves every selected layer without collapsing the combined selector", async () => {
    ...
    expect(container.querySelector('[data-slot="canvas-layer-multi-select-frame"]')).not.toBeNull();
  });
  ...
  it("keeps building canvas markup and mounts dot matrix preview when motion is enabled", async () => {
    ...
    const { container, reactRoot } = renderArtboard(motionState);
...
async function waitForQrArtboardRender() {
  await act(async () => {
    await flushPromises();
    await flushPromises();
  });
...
function createLayer(
  overrides: Partial<CanvasLayer> & Pick<CanvasLayer, "id" | "kind">,
): CanvasLayer {
