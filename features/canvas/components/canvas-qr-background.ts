...
export function buildCanvasQrBackgroundSvgPayload(
  layer: CanvasLayer,
  state: QraftyState,
...
export function getCanvasQrBackgroundSvgMarkup(layer: CanvasLayer, state: QraftyState) {
  ...
  return `<g data-canvas-qr-background="${escapeXml(shapeName)}"><defs>${defs}${clipMarkup}</defs>${contentMarkup}</g>`;
}

export function getCanvasQrBackgroundBounds(layer: CanvasLayer) {
  return {
