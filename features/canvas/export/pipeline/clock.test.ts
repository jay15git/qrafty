...
describe("export clock", () => {
  ...
  it("uses live shader frame for photo exports when paused", () => {
    expect(resolveShaderExportFrameMs({ frame: 4200, paused: true, speed: 0 }, "photo", 0)).toBe(
      4200,
    );
  });

  it("uses video clock for shader exports in video mode", () => {
    expect(resolveShaderExportFrameMs({ frame: 4200, paused: false, speed: 1 }, "video", 250)).toBe(
      250,
    );
  });
