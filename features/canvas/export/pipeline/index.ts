  ...
  const files = await Promise.all(
    items.map(async (item) => {
      const isolatedLayers = layers.map((layer) => ({
        ...layer,
        isVisible: layer.kind === "card" || layer.id === item.layerId ? layer.isVisible : false,
      }));
  ...
  const zipped = zipSync(
    Object.fromEntries(
      files.map((file) => {
        ...
        return [`${baseName}${nextCount > 1 ? `-${nextCount}` : ""}.${extension}`, file.data];
      }),
    ),
    {
