...
export const LabeledProgressIndicator: FC<LabeledProgressIndicatorProps> = ({
  ...
  return (
    <div className={cn('flex flex-col items-center gap-5', className)}>
      <div className="relative flex w-full items-center justify-center perspective-[800px] transform-3d">
        <AnimatePresence mode="popLayout">
          <motion.span
            ...
            className={cn(
              'origin-bottom flex w-full items-center justify-center text-3xl font-bold text-[#B5B5B5] will-change-transform transform-3d dark:text-zinc-400',
              labelClassName,
            )}
          >
            {labels[labelIndex]}
      ...
      <div
        className={cn(
          'h-4 w-full overflow-hidden rounded-full border border-black/5 bg-[#F0F0F0] shadow-inner dark:border-white/5 dark:bg-zinc-900',
          barClassName,
        )}
      >
        <motion.div
