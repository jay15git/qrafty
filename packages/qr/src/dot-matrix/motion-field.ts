import { AnimationPreset, type QRCodeAnimationSettings } from "./animations";
...
function ringContribution(radius: number, front: number, width: number, strength: number) {
...
function readTargetBounds(target: SVGElement) {
  ...
  const width = Number.parseFloat(target.getAttribute("width") ?? target.getAttribute("r") ?? "0");
...
function updateGradientStops(
  ...
  gradient.setAttribute("r", String(baseRadius * (1 + 0.04 * Math.sin(cyclePhase * Math.PI))));
  ...
  for (let index = 0; index <= stopCount; index += 1) {
    ...
    const color =
      isMixableHexColor(base) && isMixableHexColor(peak)
...
export function shouldUseMotionFieldLayer(preset: string, settings?: QRCodeAnimationSettings) {
