...
export const SoftMaterialize: QRCodeAnimation = (targets, x, y, count, entity) => {
  ...
  return {
    targets,
    from: entity === QRCodeEntity.Module ? Math.random() * 120 + distanceFromCenter * 4 : 120,
    duration: 520,
...
const isFinderPulseEntity = (entity: QRCodeEntity) => finderPulseEntities.indexOf(entity) > -1;
...
export const DiamondGlint: QRCodeAnimation = (targets, x, y, count, entity) => {
  ...
  return {
    targets,
    from:
      entity === QRCodeEntity.Module ? diamondDistance * 18 + sparkleOffset : diamondDistance * 8,
    duration: 640,
    easing: "cubic-bezier(0.175, 0.885, 0.320, 1.275)",
    web: {
      opacity: entity === QRCodeEntity.Module ? [1, 0.58, 1] : [1, 0.82, 1],
      scale: entity === QRCodeEntity.Module ? [1, 0.72, 1.2, 1] : [1, 0.92, 1.08, 1],
    },
  };
};
...
export const SignalScan: QRCodeAnimation = (targets, x, y, count, entity) => {
  ...
  return {
    targets,
    from:
      entity === QRCodeEntity.Module ? verticalDistance * 30 + columnOffset : verticalDistance * 16,
    duration: 700,
    easing: "cubic-bezier(0.230, 1.000, 0.320, 1.000)",
    web: {
      opacity: entity === QRCodeEntity.Module ? [1, 0.64, 1] : [1, 0.8, 1],
      scale: entity === QRCodeEntity.Module ? [1, 0.84, 1.18, 1] : [1, 0.94, 1.08, 1],
    },
  };
};
...
export const ConfettiPop: QRCodeAnimation = (targets, x, y, count, entity) => {
  ...
  return {
    ...
    web: {
      ...
      scale: entity === QRCodeEntity.Module ? [0.08, 1.36, 0.88, 1] : [0.86, 1.18, 0.97, 1],
    },
  };
};
...
export const SpiralBloom: QRCodeAnimation = (targets, x, y, count, entity) => {
  ...
  return {
    ...
    web: {
      ...
      scale: entity === QRCodeEntity.Module ? [0.12, 1.28, 0.94, 1] : [0.76, 1.12, 0.98, 1],
    },
  };
};
...
export const BubbleCascade: QRCodeAnimation = (targets, x, y, count, entity) => {
  ...
  return {
    targets,
    from: entity === QRCodeEntity.Module ? (count - y) * 22 + columnDrift + bubbleOffset : 120,
    duration: entity === QRCodeEntity.Icon ? 1040 : 820,
    easing: "cubic-bezier(0.215, 0.610, 0.355, 1.000)",
    web: {
      opacity: entity === QRCodeEntity.Module ? [0, 0.92, 1] : [1, 0.76, 1],
      scale: entity === QRCodeEntity.Module ? [0.22, 1.22, 0.9, 1.08, 1] : [0.84, 1.1, 0.97, 1],
    },
  };
};
...
export const KaleidoPulse: QRCodeAnimation = (targets, x, y, count, entity) => {
  ...
  return {
    targets,
    from:
      entity === QRCodeEntity.Module ? quadrantPulse + centerPull : Math.max(0, quadrantPulse - 80),
    duration: isFinderPulseEntity(entity) ? 980 : 760,
    easing: "cubic-bezier(0.445, 0.050, 0.550, 0.950)",
    web: {
      opacity: entity === QRCodeEntity.Module ? [1, 0.58, 1, 0.84, 1] : [1, 0.7, 1],
      scale: entity === QRCodeEntity.Module ? [1, 0.68, 1.24, 0.92, 1] : [1, 0.9, 1.14, 1],
    },
  };
};
...
export const FireflyTwinkle: QRCodeAnimation = (targets, x, y, count, entity) => {
  ...
  return {
    ...
    easing: "cubic-bezier(0.445, 0.050, 0.550, 0.950)",
    web: {
      opacity: entity === QRCodeEntity.Module ? [1, 0.48, 1, 0.72, 1] : [1, 0.74, 1, 0.88, 1],
      scale: entity === QRCodeEntity.Module ? [1, 0.76, 1.18, 0.94, 1] : [1, 0.96, 1.08, 1],
    },
  };
};
...
export const MagneticRipple: QRCodeAnimation = (targets, x, y, count, entity) => {
  ...
  return {
    ...
    web: {
      ...
      scale: entity === QRCodeEntity.Module ? [1, 0.78, 1.2, 0.96, 1] : [1, 0.9, 1.12, 1],
    },
  };
};
...
export const ParallaxTiles: QRCodeAnimation = (targets, x, y, count, entity) => {
  ...
  return {
    targets,
    from: entity === QRCodeEntity.Module ? tileDelay + tileDepth * 120 : 80 + tileDepth * 60,
...
export const ConstellationTrace: QRCodeAnimation = (targets, x, y, count, entity) => {
  ...
  const elbowOrder =
    row <= 3
      ...
      : (PREMIUM_GRID_LAST - row) * PREMIUM_GRID_SIZE + Math.abs(col - 5) * 2 + 18;
  ...
  return {
    targets,
    from:
      entity === QRCodeEntity.Module ? routeOrder * 12 + sparkleOffset : Math.min(row, col) * 36,
    duration: entity === QRCodeEntity.Icon ? 1240 : 940,
    easing: "steps(7, end)",
    web: {
      opacity:
        entity === QRCodeEntity.Module ? [1, 0.46, 1, 0.6, 1, 0.82, 1] : [1, 0.72, 1, 0.86, 1],
      scale:
        entity === QRCodeEntity.Module ? [1, 0.7, 1.22, 0.88, 1.08, 0.98, 1] : [1, 0.94, 1.08, 1],
    },
  };
};
...
export const ApertureReveal: QRCodeAnimation = (targets, x, y, count, entity) => {
  ...
  return {
    targets,
    from: entity === QRCodeEntity.Module ? edgeDepth * 130 + bladeOffset + interlockOffset : 180,
    duration: entity === QRCodeEntity.Icon ? 1040 : 860,
    easing: "steps(6, end)",
    web: {
      opacity: entity === QRCodeEntity.Module ? [0, 0.72, 1, 0.82, 1] : [0.78, 1],
      scale: entity === QRCodeEntity.Module ? [0.46, 1.12, 0.94, 1.04, 1] : [0.78, 1.12, 0.98, 1],
    },
  };
};
...
export const LensFocus: QRCodeAnimation = (targets, x, y, count, entity) => {
  ...
  return {
    targets,
    from:
      entity === QRCodeEntity.Module ? focusZone * 116 + quadrant * 34 + settleOffset : anchorLead,
    duration: isFinderPulseEntity(entity) ? 940 : 780,
    easing: "cubic-bezier(0.190, 1.000, 0.220, 1.000)",
    web: {
      opacity: entity === QRCodeEntity.Module ? [0.5, 0.86, 1, 0.78, 1] : [1, 0.66, 1, 0.9, 1],
      scale:
        entity === QRCodeEntity.Module ? [0.72, 1.14, 0.94, 1.04, 1] : [1, 1.18, 0.96, 1.06, 1],
    },
  };
};
...
export const ReceiptPrint: QRCodeAnimation = (targets, x, y, count, entity) => {
  ...
  return {
    ...
    easing: "cubic-bezier(0.230, 1.000, 0.320, 1.000)",
    web: {
      opacity: entity === QRCodeEntity.Module ? [0.14, 0.42, 1, 0.86, 1] : [0.64, 1, 0.88, 1],
      scale: entity === QRCodeEntity.Module ? [0.84, 0.96, 1.08, 0.98, 1] : [0.9, 1.08, 0.98, 1],
    },
  };
};
...
export const FlipClock: QRCodeAnimation = (targets, x, y, count, entity) => {
  ...
  return {
    targets,
    from: entity === QRCodeEntity.Module ? panelOrder * 48 + columnAlternate + splitOffset : 140,
    duration: entity === QRCodeEntity.Icon ? 1040 : 760,
    easing: "cubic-bezier(0.175, 0.885, 0.320, 1.275)",
    web: {
      opacity: entity === QRCodeEntity.Module ? [0.22, 0.7, 1, 0.88, 1] : [0.72, 1, 0.86, 1],
      scale: entity === QRCodeEntity.Module ? [0.58, 1.2, 0.82, 1.08, 1] : [0.84, 1.12, 0.96, 1],
    },
  };
};
...
export const WaveInterference: QRCodeAnimation = (targets, x, y, _count, entity) => {
  ...
  return {
    ...
    easing: "cubic-bezier(0.445, 0.050, 0.550, 0.950)",
    web: {
      opacity: entity === QRCodeEntity.Module ? [0.42, 0.78, 1, 0.86, 1] : [0.74, 1, 0.9, 1],
      scale: entity === QRCodeEntity.Module ? [0.88, 1.14, 0.96, 1.05, 1] : [0.94, 1.08, 1],
    },
  };
};
...
export const QuantumMaterialize: QRCodeAnimation = (targets, x, y, count, entity) => {
  ...
  return {
    ...
    easing: "steps(8, end)",
    web: {
      opacity: entity === QRCodeEntity.Module ? [0, 1, 0.15, 1, 0.4, 1] : [0.5, 1, 0.7, 1],
      scale: entity === QRCodeEntity.Module ? [0.7, 1.1, 0.9, 1.04, 1] : [0.9, 1.06, 1],
      x:
        entity === QRCodeEntity.Module
      ...
      filter:
        entity === QRCodeEntity.Module
          ? ["brightness(1.35)", "brightness(1.65)", "brightness(0.88)", "brightness(1)"]
...
export const MagneticSnap: QRCodeAnimation = (targets, x, y, count, entity) => {
  ...
  const scatterX = (hashNoise(x, y, 11) - 0.5) * (entity === QRCodeEntity.Module ? 22 : 10);
  const scatterY = (hashNoise(x, y, 37) - 0.5) * (entity === QRCodeEntity.Module ? 22 : 10);
  const spin = (hashNoise(x, y, 53) - 0.5) * (entity === QRCodeEntity.Module ? 48 : 20);
  ...
  return {
    ...
    easing: "cubic-bezier(0.175, 0.885, 0.320, 1.275)",
    web: {
      opacity: entity === QRCodeEntity.Module ? [0.2, 0.6, 1, 0.92, 1] : [0.65, 1, 0.9, 1],
      scale: entity === QRCodeEntity.Module ? [0.5, 1.24, 0.92, 1.06, 1] : [0.82, 1.12, 1],
      x:
        entity === QRCodeEntity.Module
      ...
      y:
        entity === QRCodeEntity.Module
          ...
          : [scatterY * 0.4, 0],
      rotate:
        entity === QRCodeEntity.Module ? [spin, spin * 0.4, -spin * 0.15, 0] : [spin * 0.5, 0],
      filter:
        entity === QRCodeEntity.Module
          ? [
...
export const HoloFlicker: QRCodeAnimation = (targets, x, y, count, entity) => {
  ...
  return {
    ...
    easing: "cubic-bezier(0.445, 0.050, 0.550, 0.950)",
    web: {
      opacity: entity === QRCodeEntity.Module ? [1, 0.44, 1, 0.68, 1, 0.88, 1] : [1, 0.78, 1],
      scale: entity === QRCodeEntity.Module ? [1, 1.08, 0.94, 1.12, 0.98, 1.04, 1] : [1, 1.04, 1],
      filter:
        entity === QRCodeEntity.Module
          ? [
...
export const SignalGlitch: QRCodeAnimation = (targets, x, y, count, entity) => {
  ...
  const jumpX = (hashNoise(x, y, seed % 41) - 0.5) * (entity === QRCodeEntity.Module ? 16 : 8);
  ...
  return {
    ...
    easing: "steps(6, end)",
    web: {
      opacity: entity === QRCodeEntity.Module ? [0.35, 1, 0.2, 1, 0.55, 1] : [0.7, 1, 0.85, 1],
      scale: entity === QRCodeEntity.Module ? [1, 1.06, 0.94, 1.02, 1] : [1, 1.04, 1],
      x: entity === QRCodeEntity.Module ? [jumpX, -jumpX * 0.7, jumpX * 0.4, 0] : [jumpX * 0.5, 0],
      filter:
        entity === QRCodeEntity.Module
          ? [
...
export const ShockwaveJolt: QRCodeAnimation = (targets, x, y, _count, entity) => {
  ...
  return {
    ...
    easing: "cubic-bezier(0.445, 0.050, 0.550, 0.950)",
    web: {
      opacity: entity === QRCodeEntity.Module ? [0.3, 1, 0.65, 1] : [0.75, 1, 0.92, 1],
      scale: entity === QRCodeEntity.Module ? [0.82, punch, 0.85, 1.08, 1] : [0.92, punch, 0.96, 1],
      filter:
        entity === QRCodeEntity.Module
          ? ["brightness(0.9)", "brightness(1.55)", "brightness(1.08)", "brightness(1)"]
...
export const TideRise: QRCodeAnimation = (targets, x, y, count, entity) => {
  ...
  return {
    targets,
    from: entity === QRCodeEntity.Module ? riseDelay + waveOffset : riseDelay * 0.3,
    duration: entity === QRCodeEntity.Icon ? 1000 : 760,
    easing: "cubic-bezier(0.190, 1.000, 0.220, 1.000)",
    web: {
      opacity: entity === QRCodeEntity.Module ? [0.28, 0.72, 1, 0.92, 1] : [0.76, 1, 0.9, 1],
      scale: entity === QRCodeEntity.Module ? [0.72, 1.12, 0.96, 1.06, 1] : [0.88, 1.1, 1],
      y: entity === QRCodeEntity.Module ? [riseDistance, -2, 1, 0] : [riseDistance * 0.5, 0],
    },
  };
};
...
export const GravityCollapse: QRCodeAnimation = (targets, x, y, count, entity) => {
  ...
  return {
    ...
    easing: "cubic-bezier(0.190, 1.000, 0.220, 1.000)",
    web: {
      opacity: entity === QRCodeEntity.Module ? [0.12, 0.52, 0.92, 1, 0.96, 1] : [0.55, 0.9, 1],
      scale: entity === QRCodeEntity.Module ? [0.4, 0.78, 1.16, 0.94, 1.04, 1] : [0.72, 1.08, 1],
      x:
        entity === QRCodeEntity.Module
      ...
      y:
        entity === QRCodeEntity.Module
          ...
          : [startY * 0.5, 0],
      rotate:
        entity === QRCodeEntity.Module ? [spin, spin * 0.35, -spin * 0.1, 0] : [spin * 0.4, 0],
    },
  };
};
