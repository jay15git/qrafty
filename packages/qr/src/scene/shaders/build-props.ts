...
export type PaperShaderParamValue =
  boolean | number | number[] | number[][] | string | string[] | undefined;
...
export function buildPaperShaderRenderProps(
  ...
  const frame = options?.frameMs !== undefined ? options.frameMs : shader.frame;
  const speed = options?.seek || shader.paused ? 0 : shader.speed;
